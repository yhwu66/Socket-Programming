...
#define MYPORT "21483" // the port users will be connecting to
...
int main(void)
{
	...
	FILE *fp1;
	fp1 = fopen("block1.txt", "r");
	FILE *fp1_tmp;
	;
	fp1_tmp = fopen("block1_tmp.txt", "w");
	char *line_buf = NULL;
	...
	fclose(fp1_tmp);
	remove("block1.txt");
    rename("block1_tmp.txt", "block1.txt");
